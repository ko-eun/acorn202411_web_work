...
<%
	...
	// 2. id 에 해당하는 회원 정보를 UserDao 객체를 이용해서 얻어와서
	UserDto dto = UserDao.getInstance().getData(userName);
	
	// 3. 실제로 존재하는 아이디이고 존재한다면 비밀번호도 일치하는지 비교해서
	boolean isLoginSuccess = false;
	if(dto !=null){// null이 아니므로 실제 존재하는 아이디이다
		// 비밀번호가 일치하는지 확인해서 일치한다면 로그인 처리를 한다
		if(dto.getPassword().equals(password)){
			//로그인 처리를 한다(로그인 된 user 정보를 session 영역에 담는다)
