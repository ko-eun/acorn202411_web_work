...
public class LoginFilter implements Filter{
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        // session 영역에서 로그인 된 정보를 얻어내기 위한 객체
        HttpSession session = req.getSession();
        // session 영역에 sessioDto 라는 키값으로 저장된 값이 있으면 얻어내서 원래 type 으로 casting
        SessionDto dto = (SessionDto) session.getAttribute("sessionDto");
        // 만일 로그인을 하지 않았다면
        if (dto == null) { 
        	// 로그인 페이지로 리다일렉트(요청을 다시하라고 응답을 하는 것) 시키는 메소드를 호출해서 리다일렉트로 시킨다
    ...
    private void redirectToLogin(HttpServletRequest req, HttpServletResponse res) throws IOException {
        ...
        // 혹시 뒤에 query parameter 가 있다면 그 역시 읽어온다.   ?num=xxx&count=xxx.
